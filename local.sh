...
# Change parameter token denominations to ucist
cat $HOME/.cist/config/genesis.json | jq '.app_state["staking"]["params"]["bond_denom"]="ucist"' > $HOME/.cist/config/tmp_genesis.json && mv $HOME/.cist/config/tmp_genesis.json $HOME/.cist/config/genesis.json
cat $HOME/.cist/config/genesis.json | jq '.app_state["crisis"]["constant_fee"]["denom"]="ucist"' > $HOME/.cist/config/tmp_genesis.json && mv $HOME/.cist/config/tmp_genesis.json $HOME/.cist/config/genesis.json
cat $HOME/.cist/config/genesis.json | jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="ucist"' > $HOME/.cist/config/tmp_genesis.json && mv $HOME/.cist/config/tmp_genesis.json $HOME/.cist/config/genesis.json
cat $HOME/.cist/config/genesis.json | jq '.app_state["mint"]["params"]["mint_denom"]="ucist"' > $HOME/.cist/config/tmp_genesis.json && mv $HOME/.cist/config/tmp_genesis.json $HOME/.cist/config/genesis.json
cat $HOME/.cist/config/genesis.json | jq '.app_state["evm"]["params"]["evm_denom"]="ucist"' > $HOME/.cist/config/tmp_genesis.json && mv $HOME/.cist/config/tmp_genesis.json $HOME/.cist/config/genesis.json
...
cistd add-genesis-account $KEY 100000000000000000000000000ucist --keyring-backend $KEYRING

# Sign genesis transaction
cistd gentx $KEY 1000000000000000000000ucist --keyring-backend $KEYRING --chain-id $CHAINID
...
cistd start --pruning=nothing --evm.tracer=json $TRACE --log_level $LOGLEVEL --minimum-gas-prices=0.0001ucist --json-rpc.api eth,txpool,personal,net,debug,web3,miner --api.enable